...
def location_matches(job_location, requested_location):
    ...
    requested_location = normalize_text(requested_location)
    if requested_location == "remote":
        return "remote" in job_location
    return requested_location in job_location

def calculate_role_score(job_title, requested_role):
    if not requested_role:
        return 0.0
    title = normalize_text(job_title)
    role = normalize_text(requested_role)
    if title == role:
        return 1.0
    if role in title:
        return 0.9
...
def calculate_skill_score(job,requested_skills):
    if not requested_skills:
        return 0.0
    job_text = normalize_text((str(job.get("title", "")) + " " + str(job.get("description", ""))))
    matches = 0
    for skill in requested_skills:
        normalized_skill = (normalize_text(skill))
        if normalized_skill in job_text:
            matches += 1
    return (matches / len(requested_skills))
...
def semantic_job_search(query,top_k=10):
    intent = parse_job_intent(query)
    role = intent["role"]
    location = intent["location"]
    skills = intent["skills"]
    semantic_parts = []
    if role:
        semantic_parts.append(role)
    if skills:
        semantic_parts.extend(skills)
    if semantic_parts:
    ...
    try:
        ...
        if current_user:
            stored_profile = get_profile_embedding(
            current_user["user_id"]
            )
            if stored_profile:
                profile_embedding = stored_profile.get("embedding")
    except Exception:
        profile_embedding = None
    ...
    for row in embeddings:
        ...
        if job is None:
            continue
        if not location_matches(job.get("location"), location):
            continue
        semantic_score = (cosine_similarity(query_embedding, embedding))
        if profile_embedding is not None:
            profile_score = cosine_similarity(profile_embedding,embedding,)
        else:
            profile_score = 0.0
        role_score = (calculate_role_score(job.get("title"),role))
        skill_score = (
        ...
        final_score = (semantic_score * 0.50 + profile_score * 0.25 + role_score * 0.15 + skill_score * 0.10)
        matches.append(
            {
            "job_id": job_id,
    ...
    for match in matches:
        ...
        result = dict(job)
        result["similarity"] = match["semantic_score"]
        result["profile_score"] = match["profile_score"]
        result["final_score"] = match["final_score"]
        result["role_score"] = match["role_score"]
