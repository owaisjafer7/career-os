...
Instead of relying only on keyword search, CareerOS combines
**job-description embeddings** with a user's career profile, skills,
target role, location, work-mode preference, salary expectations, and
experience. The application can search and rank jobs, explain match
quality, save jobs, track application status, generate tailored
application materials, prepare users for interviews, and surface stale
applications.

### ✅ Capstone Requirements Covered

  -----------------------------------------------------------------------
  Requirement                         CareerOS
  ----------------------------------- -----------------------------------
  ⚡ Spark data pipeline              Adzuna ingestion + downstream
                                      transformations

  🌐 Third-party API                  Adzuna Jobs API

  📄 Unstructured data                Job descriptions + resume/profile
                                      text

  🧠 Semantic retrieval               Sentence Transformer embeddings +
                                      cosine similarity

  🖥️ Databricks App                   Streamlit frontend

  🗄️ Lakebase                         Profiles, skills, jobs,
                                      applications, notes

  🤖 AI agent                         Tool-calling Databricks-served
                                      model

  ✍️ Real agent actions               Save jobs + update application
                                      status
  -----------------------------------------------------------------------
...
### Search Ranking Formula
  ...
  Profile similarity            **25%**
  Role relevance                **15%**
  Requested-skill relevance     **10%**
...
> **Retrieval answers:** "What should I see?"\
> **Match analysis answers:** "How well does this job fit me?"
...
## Tables
  ...
  Table               Purpose
  ------------------- ------------------------------------------------
  `users`             CareerOS users
  `profiles`          Career preferences and resume context
  `skills`            Skills, proficiency, and experience
  `job_postings`      Jobs required by transactional/agent workflows
  `saved_jobs`        Saved opportunities
  `applications`      Application pipeline state
  `interview_notes`   Notes tied to applications
  `contacts`          Career/network contacts
...
## 🛠️ Agent Tools
  ...
  `get_stale_applications`      Find neglected          📖 Read
                                applications            

  `draft_cover_letter`          Retrieve grounded       📖 Read
                                job/profile context     

  `draft_resume_bullets`        Retrieve grounded       📖 Read
                                job/profile context     

  `prepare_interview`           Retrieve grounded       📖 Read
                                interview context       
  -----------------------------------------------------------------------------
...
## Key Modules
  ...
  ------------------------ ---------------------------------------------------
  `app.py`                 Streamlit frontend and user workflows
  `ai_agent.py`            Agent prompt, tools, execution, context injection
  `job_search.py`          Semantic/profile search and hybrid ranking
  `job_intent.py`          Natural-language job-intent parsing
  `profile_embedding.py`   Candidate-profile embeddings
  `lakebase.py`            Low-level PostgreSQL/Lakebase access
  `lakebase_actions.py`    CareerOS-specific reads and writes
...
# 🏆 Capstone Requirement Coverage
  ...
  ------------------- -------------------- --------------------------------
  Spark data pipeline Adzuna → Bronze →                   ✅
                      Silver               

  Third-party API     Adzuna Jobs API                     ✅

  Unstructured data   Job descriptions +                  ✅
                      resume/profile text  

  Semantic retrieval  Embeddings + cosine                 ✅
                      similarity           

  Databricks App      Streamlit CareerOS                  ✅
                      frontend             
  ...
  AI agent            Databricks model +                  ✅
                      CareerOS tools       

  Agent reads         Search, saved jobs,                 ✅
                      stale applications,  
                      context retrieval    

  Agent writes        Save jobs + update                  ✅
                      application status   

  Personalized AI     Profile + skills +                  ✅
                      preferences +        
                      profile embedding    
  -------------------------------------------------------------------------

> **CareerOS covers the core architectural requirements of the AI Job
> Hunting Copilot capstone.**
...
**1. Build a profile**\
Store target role, location, salary expectations, experience, resume
context, and skills.
...
**3. Rank personally**\
Jobs receive query-similarity and profile-similarity signals.

**4. Analyze a job**\
CareerOS calculates a multi-factor match score and explains strengths
and gaps.

**5. Save the opportunity**\
The user or agent persists the job to Lakebase.

**6. Tailor the application**\
CareerOS retrieves the actual posting and profile before generating
materials.
...
Generate job-specific preparation grounded in the posting and candidate
background.

**9. Follow up**\
Stale-application logic identifies opportunities that may need
attention.
...
10. Launch/deploy the Streamlit Databricks App.
11. Create a career profile and skills.
12. Search, analyze, save, track, and interact with jobs through
    CareerOS.
...
The result is an application where AI is connected to both an
**analytical job corpus** and **persistent transactional state**.
...
# 🧰 Tech Stack
  ...
  Storage                  Delta Lake
  Transactional Database   Lakebase / PostgreSQL
  AI                       DatabricksOpenAI
  Model Integration        Databricks SDK
...
deduplication, and refreshable data.
...
Databricks AI/Data Engineering ecosystem and an architecture centered on
**Spark, Lakebase, unstructured retrieval, Databricks Apps, and AI
agents**.
